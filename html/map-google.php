    ...
    <script>
        ...
        function initMap(position) {
            ...
            routingControl.on('routesfound', function(e) {
                ...
                // Listen for when the user moves along the route
                map.on('moveend', function() {
                    var currentPos = map.getCenter();
                    var nextInstruction = instructions[nextInstructionIndex];

                    // Check if the user is near the next instruction point
                    if (nextInstruction && map.distance(currentPos, nextInstruction.latLng) < 50) {
            ...
            // Click event to set a destination
            map.on('click', function(e) {
                var destinationLatLng = e.latlng;
                if (routingControl.getWaypoints().length > 1) {
                    routingControl.spliceWaypoints(routingControl.getWaypoints().length - 1, 1, destinationLatLng);
                } else {
                    routingControl.spliceWaypoints(routingControl.getWaypoints().length, 0, destinationLatLng);
                }
            });
        }
